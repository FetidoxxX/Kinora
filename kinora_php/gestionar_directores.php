...
switch ($action) {
    case 'crear_director':
        ...
        if ($nombre === '' || $apellido === '') {
            echo json_encode(['status' => 'error', 'message' => 'El nombre y el apellido del director son requeridos.']);
            exit;
        }
        ...
        $q = "SELECT id_director FROM director WHERE nombre = '$safe_nombre' AND apellido = '$safe_apellido' LIMIT 1";
        $res = mysqli_query($conn, $q);
        
        if ($res === false) {
            echo json_encode(['status' => 'error', 'message' => 'Error en consulta de verificación de director: ' . mysqli_error($conn)]);
            exit;
        }
        ...
        $ins = "INSERT INTO director (nombre, apellido) VALUES ('$safe_nombre', '$safe_apellido')";
